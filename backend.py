...
# Utility: Recursively convert all datetime fields in a dict to ISO strings
def convert_datetimes_to_iso(data):
    if isinstance(data, dict):
        return {k: convert_datetimes_to_iso(v) for k, v in data.items()}
    elif isinstance(data, list):
        return [convert_datetimes_to_iso(i) for i in data]
...
class LLMService:
    ...
    def generate_interview_questions(self, job_description: str, experience_level: str) -> List[str]:
        """Generate technical interview questions based on job description (using a fallback if the LLM returns a code block)."""
        try:
            prompt = (
